...
// ---------------------------------------------------------------------------
HINSTANCE globalModuleHandle = 0;
    ...
    , fMsgDomain(0)
{
    ...
    if (!fModHandle)
    {
        ...
        if (!fModHandle)
        {
			// Übernommen aus Rev 17550 (Xerces 2.7):
			// Ralfs Bugfix, um die Ressourcen auch bei statischem Linken zu laden.
			fModHandle = ::GetModuleHandle( NULL );
